...
import { useGetCostsTotalGroupedByCategory } from "@appHooks";
...
export function PieDiagram(props) {
  ...
  React.useEffect(() => {
    ...
    if (totalByCategoriesValue) {
      ...
      const values = category
        ? Object.values(totalByCategoriesValue).filter((o) => {
  ...
  return Boolean(state?.labels?.length) ? (
    ...
    <p className="pie-no-costs font-tech ">
      You haven't created any cost spending yet
    </p>
  );
}
